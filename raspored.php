...
function postavi_staticki_raspored(&$raspored, $predmeti, $odeljenja, $ogranicenja_nastavnika) {
    ...
    foreach ($odeljenja as $odeljenje) {
        ...
        foreach ($dani as $dan) {
            $satovi = array_keys(array_filter($raspored[$dan][$odeljenje], function($cas) {
                return $cas === null;
            }));
            ...
            foreach ($satovi as $index => $sat) {
                if (isset($rotacija[$index])) {
                    ...
                    // Logovanje predmeta koji se postavlja
                    echo "Postavljam predmet: $predmet na $dan, sat $sat\n";

                    if (validno_postavljanje($raspored, $dan, $sat, $predmet, $odeljenje, $ogranicenja_nastavnika)) {
                        postavi_cas_bez_pauze($raspored, $dan, $sat, $predmet, $odeljenje);
                    }
                }
            }
        }
